...
class ASTGCN(torch.nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        c_in = kwargs.get('step_num_in', 6)
        c_out = kwargs.get('step_num_out', 6)
        ...
        K = kwargs.get('K', 2)
        adj_mx_path = kwargs.get('adj_path', './data/PEMS04/adj_mx.npy')
    ...
    def forward(self, x):
        x = x['flow_x'].to(device)
        y = self.blocks(x)
        y = self.final_conv(y).permute(0, 2, 1, 3)
        # print("y:", y.size())
        return y
