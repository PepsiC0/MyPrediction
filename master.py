...
parser.add_argument('--input_size', type=int, default=config['model']['input_size'], help='输入的时间步')
parser.add_argument('--hidden_size', type=int, default=config['model']['hidden_size'], help='隐层大小')
parser.add_argument('--num_layers', type=int, default=config['model']['num_layers'], help='隐层个数')
parser.add_argument('--output_size', type=int, default=config['model']['output_size'], help='预测时间步')
...
def main():
    ...
    log_string(log, "The shape of  adjacency matrix: {}".format(adj.shape))
    ...
    # 设置日志文件的路径和文件名
    log_dir = "logs/PEMS04/LSTM"  # 指定日志文件夹的路径和名称
    ...
    for i in tqdm.tqdm(range(1, args.epochs + 1)):
        ...
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            # writer.add_scalar('train_loss', loss, iter + 1)
        ...
        log_string(log, logs.format(i, mtrain_loss, mtrain_mae, mtrain_mape, mtrain_rmse, mvalid_loss, mvalid_mape,
                                    mvalid_rmse, (t2 - t1)))
    ...
    for t in range(args.horizon):
        ...
        # 保存结果
        save_dir = './plots/PEMS04/LSTM'
        pre_dir = 'predicted.npy'
        test_dir = 'test.npy'
        result_file = './plots/PEMS04/LSTM/result.h5'

        pred = pred.cpu().numpy()
        real = realy.cpu().numpy()

        # 创建保存目录（如果不存在）
        os.makedirs(save_dir, exist_ok=True)
        np.save(os.path.join(save_dir, pre_dir), pred)
        np.save(os.path.join(save_dir, test_dir), real)


        file_obj = h5py.File(result_file, "w")  # 将预测值和目标值保存到文件中，因为要多次可视化看看结果
        file_obj["predict"] = pred  # [N, T, D]
        file_obj["target"] = real  # [N, T, D]
